...
const CookieConsentBanner = () => {
  ...
  const handleAcceptAll = async () => {
    ...
    try {
      ...
      if (posthog) {
        trackError(posthog, 'consent_accept_all_failed', { error: error.message });
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const handleNecessaryOnly = async () => {
    ...
    try {
      ...
      if (posthog) {
        trackError(posthog, 'consent_necessary_only_failed', { error: error.message });
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (posthog) {
        trackError(posthog, 'consent_save_failed', { error: error.message });
      }
    } finally {
      setIsLoading(false);
    }
