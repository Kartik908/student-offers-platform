    ...
    <AnimatePresence>
      {isVisible && (
        <motion.button
          ...
          whileHover={{ scale: isMobile ? 1.04 : 1.08, y: isMobile ? -2 : -4 }}
          whileTap={{ scale: 0.92 }}
          className="fixed bottom-6 right-4 sm:bottom-8 sm:right-7 z-50 flex flex-col items-center justify-center gap-0.5 rounded-full border border-white/50 dark:border-white/10 bg-gradient-to-b from-white/90 to-white/50 dark:from-neutral-900/90 dark:to-neutral-900/50 hover:from-white hover:to-white/60 dark:hover:from-neutral-800/90 dark:hover:to-neutral-800/60 text-neutral-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-300 shadow-[0_8px_32px_rgba(0,0,0,0.12)] dark:shadow-black/20 hover:shadow-[0_12px_40px_rgba(0,0,0,0.15)] dark:hover:shadow-black/30 ring-1 ring-black/5 dark:ring-white/10 transition-all duration-200 p-2 sm:p-2.5 w-7 h-18 sm:w-8 sm:h-20 backdrop-blur-xl supports-[backdrop-filter]:backdrop-blur-xl focus-visible:ring-2 focus-visible:ring-blue-300 focus-visible:ring-offset-2 touch-manipulation"
        >
          <ArrowUp className="w-2.5 h-2.5 sm:w-3 sm:h-3 stroke-[2.5] dark:drop-shadow-[0_1px_2px_rgba(0,0,0,0.3)]" />
          <span className="text-[8px] sm:text-[9px] font-semibold tracking-wide rotate-180 [writing-mode:vertical-lr] dark:drop-shadow-[0_1px_2px_rgba(0,0,0,0.3)]">
            TO TOP
          </span>
        </motion.button>
      )}
