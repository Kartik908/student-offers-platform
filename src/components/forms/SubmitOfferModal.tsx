...
export function SubmitOfferModal() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    track('submit_offer_form_submitted', {
      url: values.url,
    ...
    try {
      // Submit to Supabase
      const result = await submissionsService.submitOffer({
        url: values.url,
        category: values.category,
      });
    ...
    <Dialog open={isSubmitOfferModalOpen} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="max-w-lg max-h-[85vh] p-0 gap-0 flex flex-col">
        ...
        <div className="px-6 pt-6 pb-4 space-y-1 flex-shrink-0">
          <h2 className="text-xl font-semibold">Share a Student Offer</h2>
        ...
        {/* Footer */}
        <div className="flex justify-end gap-3 p-4 flex-shrink-0 bg-background">
          <Button
            variant="secondary"
            onClick={handleClose}
