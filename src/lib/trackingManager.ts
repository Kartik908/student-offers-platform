...
class TrackingManager {
  ...
  track(eventName: string, properties?: Record<string, unknown>) {
    ...
    if (isNecessaryEvent) {
      (this.posthog as { capture: (event: string, properties?: Record<string, unknown>) => void }).capture(eventName, {
        ...properties,
  ...
  /**
   * Track errors (always allowed)
   */
  trackError(error: string | Error, context?: Record<string, unknown>) {
  ...
  identify(userId: string, properties?: Record<string, unknown>) {
    ...
    (this.posthog as { identify: (userid: string, properties?: Record<string, unknown>) => void }).identify(userId, {
      ...safeProperties,
  ...
  setUserProperties(properties: Record<string, unknown>) {
    ...
    (this.posthog as { people: { set: (properties: Record<string, unknown>) => void } }).people.set(safeProperties);
  }
...
export const trackError = (error: string | Error, context?: Record<string, unknown>) => {
  trackingManager.trackError(error, context);
};
