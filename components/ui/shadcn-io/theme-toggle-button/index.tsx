  ...
  const handleClick = useCallback(() => {
    ...
    if (variant === 'circle') {
      ...
      css = `
        ...
        /* Fallback for Brave and other browsers without View Transitions API */
        @supports not (view-transition-name: root) {
          html {
            animation: circle-expand-fallback 0.4s ease-out;
          }
          @keyframes circle-expand-fallback {
            from {
              clip-path: circle(0% at ${cx}% ${cy}%);
            }
            to {
      ...
      css = `
        @supports (view-transition-name: root) {
          ::view-transition-old(root) { 
            animation: fade-out-smooth 0.3s ease-out;
          }
          ::view-transition-new(root) {
            animation: circle-blur-expand-smooth 0.6s cubic-bezier(0.4, 0, 0.2, 1);
            transform-origin: ${positions[start]};
          }
          ...
          @keyframes circle-blur-expand-smooth {
            ...
            30% {
              clip-path: circle(40% at ${cx}% ${cy}%);
              filter: blur(4px);
              opacity: 0.9;
            }
            70% {
              clip-path: circle(80% at ${cx}% ${cy}%);
              filter: blur(1px);
              opacity: 0.95;
            }
            100% {
